...
export default async function CompanyDetailPage({ params }: {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/company/detail/${slug}`);
  const data = await res.json();
  const { companyDetail, jobList } = data;
        ...
        <div className="pt-[30px] pb-[60px]">
          <div className="container mx-auto px-[16px]">
            ...
            <div className="border border-[#DEDEDE] rounded-[8px] p-[20px]">
              ...
              <div className="flex flex-col gap-[10px]">
                <div className="font-[400] text-[16px] text-[#A6A6A6]">
                  Mô hình công ty:
                  <span className="text-[#121212] ml-[10px]">
            ...
            <div className="border border-[#DEDEDE] rounded-[8px] p-[20px] mt-[20px]">
              <div className="text-[24px] font-bold mb-[20px]">Mô tả chi tiết</div>
              <div dangerouslySetInnerHTML={{ __html: companyDetail.description }}></div>
            </div>
            {/* Hết Mô tả chi tiết */}
            ...
            <div className="mt-[30px]">
              <h2 className="font-[700] text-[28px] text-[#121212] mb-[20px]">
                Số công việc: {jobList.length}
              </h2>

              <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-[20px]">
                {jobList && jobList.length && jobList.map((item: any, index: number) => (
                  <CardJobItem
                    job={item}
                    key={index}
                  />
